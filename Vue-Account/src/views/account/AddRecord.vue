...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getAccountTypeList() {
        let sessionExpend = sessionStorage.getItem("account-types-expend");
        let sessionIncome = sessionStorage.getItem("account-types-income");

        if (sessionExpend && sessionIncome) {
          let expendTypes = JSON.parse(sessionExpend);
          let incomeTypes = JSON.parse(sessionIncome);
